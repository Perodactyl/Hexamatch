...
#			foundRoot = true
		if (root || touchingroot) && c.is_colliding():
			c.get_collider().get_parent().touchingroot = true
		if c.is_colliding() and c.get_collider().get_parent().position == position:
	...
	if deadFrames >= 60 && !$kill.is_active():
		$kill.interpolate_property(self, "position", position, Vector2(position.x, 600), rand_range(0.5, 4))
		killAnim()
		Score.add(500)
...
func recursiveGetAmt(found:Array):
	for cast in $casting.get_children():
		if cast.is_colliding():
			...
			if hex.color == color && !(hex in found):
				found.append(hex)
